function [] = doSim1E(T,t,sigmai,sigmaiy, mui, muiy, Ri, RB, Pi, RD1, RD2, RD3, RD4, PB1, PB2, PB3, PB4,PiCarry,RiCarry)
%DOSIM1E Creates and save the requested sim file
%   should handle file type by it self.
...
if ispc
    savepath = strcat("Data\55555\X_",string(mui),'_Y_',string(muiy),'\Tau_',string(RiCarry),'\rate_',string(PiCarry),'\');
    mkdir(savepath)
    addpath(genpath('Data\'))
    temppath = cd(savepath);
    save(strcat(filetype,'.mat'),"sync","ch","dtime","total")
    cd(temppath)
elseif isunix || ismac
    savepath = strcat("Data/55555/X_",string(mui),'_Y_',string(muiy),'/Tau_',string(RiCarry),'/rate_',string(PiCarry),'/');
    mkdir(savepath)
